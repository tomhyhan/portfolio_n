...
status: 'published'
tags: 'oauth,nextjs,context'
postTitle: 'Oauth & Next.js - Part1'
category: 'dev'
---

OAuth is a way for you to delegate the responsibility of managing user data to (usally well-known) third party services like Google, FaceBook and etc. Both you and users can benefit from Oauth - Users generally feel more secure with this approach becuase they don't have to share passwords with you, and you don't have to worry about storing passwords securely. 

In part1 of OAuth series, we will dive into how to implement Google Oauth with Next.js. I tried to make this post as simple and light as possible, so I will limit the use of external libraries and CSS.

### Creating OAuth Credentials

First, we need to create OAuth credentials for our app. Go to [Google Cloud Console](https://console.cloud.google.com/) and create a new project. Then, go to the [credentials page](https://console.cloud.google.com/apis/credentials) and create a new OAuth client ID. In your callback URL, add `http://localhost:3000/api/auth/callback`.

Save your `client ID` and `secret` somewhere for later use.
...
The First step is to get a permission from the user to access their data. We will create an API route to handle this. 
...
```
Here, we are creating a new OAuth2 client with the credentials and callback ULR we created earlier.

Next, create a file `route.ts` in a folder `api/auth/redirect` and add the following code:
...
After the user grants permission, Google will redirect the user to our callback URL with a `code` information. We will use this `code` to get an access token from Google.
...
```
<Information>
    In this step, you could also save the user information in a database
</Information>
...
Here's what's happening in the code above:
    ...
    3. We set the access token in a cookie by setting http only to true and redirect the user to the home page.
...
In the above code, we:
    ...
    2. Defined AuthProvider component that will wrap our application. This component will maintain `user` and `loading` state.
    3. The first user visits our website, we use `useEffect` to get the user data from our server (this will be explained shortly). 
...
```
<Information>
    There are multiple ways to get the user data in differenct ways. For example, if you were to use JWT Token, then you could decode the token to get the user data. If you were to use a database, then you could query the database to get the user data. For simplicity, we use Google API directly to get the user data.
</Information>
...
The basic logic set up, we are now ready to create our home page. 
...
We will create a protected page that only logged in users can access.
...
There are few thing that we need to consider with our implementation.
    ...
    3. CSRF Token: Right Now, we have no way of knowing if the request is coming from our website or not. When we set a cookie, we could also set a CSRF token and check if the request has the same CSRF token.

## Summary

In this post, we have built a Next.js app can you use Google OAuth to login and logout. We created apis to handle the login and logout logic. We also created a context to manage the user data. Lastly, we created a protected page that only logged in users can access.

In the next post, we abstract away out authentication login with NextAuth.js.
