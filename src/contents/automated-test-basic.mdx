...
With the caculator project, we have familiarized ourselves with `Matchers` in `Jest`. Now, let's create another projects to learn about how we can isolate our tests by seperating the dependencies.
...
This time, we are going to create a Music service that fetches the music data from the server. Create a `mock` folder and inside, create a file `MusicService.js` and `MusicClient.js`
...
The `MusicService` class takes a `MusicClient` as a dependency and fetches the favorite music data from the server. The `getFavoriteMusic` method fetches the data once and caches it. The subsequent calls to the method will return the cached data instead of making a request to the server.

Note that we have seperated the api request and the business logic, so that we can test the business logic without worrying about the api request.
...
The above test runs as follows:

    1. Import the `MusicClient` and `MusicService` classes.
    2. Mock the `MusicClient` class with `Jest` mock.
    3. Mock a `get` function since the `MusicService` class depends on the `get` function.
    4. Implement the `MusicClient` class with mock `get` function.
    5. Before each test, create a new instance of `MusicService` class.
    6. In the first test, call the `getFavoriteMusic` method and check if the result of the mock `get` function matches with the `favoriteMusic`.
    7. In the second test, call the `getFavoriteMusic` method twice and check if the mock `get` function is called only once.
    8. After each test, make sure to clear the mock functions.
...
Stub is a dummy/fake object that can be used in place of an actual object (dependency). Since we controll the behavior of the stub, we can use it simplify our tests.
